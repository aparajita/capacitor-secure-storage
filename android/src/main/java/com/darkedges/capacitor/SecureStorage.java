...
public class SecureStorage extends Plugin {
  ...
  private String getDataFromPreferences(String prefixedKey)
  ...
  private boolean removeDataFromPreferences(String prefixedKey)
    throws GeneralSecurityException, IOException {
    try {
      SSSecurityUtilsFactory.getSSSecurityUtilsInstance()
        .deleteKey(prefixedKey);
  ...
  private ArrayList<String> getKeys(String prefix)
    throws GeneralSecurityException, IOException {
    try {
      return SSSecurityUtilsFactory.getSSSecurityUtilsInstance()
        .getKeys(prefix);
    } catch (SSSecurityException e) {
      throw new GeneralSecurityException(e);
    }
  ...
  private String encryptString(String str, String prefixedKey)
    ...
    try {
      return SSSecurityUtilsFactory.getSSSecurityUtilsInstance()
        .encode(getContext(), prefixedKey, str, false);
    } catch (SSSecurityException e) {
      throw new GeneralSecurityException(e);
    }
  }
